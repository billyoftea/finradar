...
def _run_market(mode="all"):
    """运行市场数据/社交媒体抓取"""
    import asyncio
    from finradar.market.tracker import MarketTracker, main as market_main

    asyncio.run(market_main(mode_override=mode))
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="finradar - 热点新闻聚合与金融市场追踪",
...
运行模式:
  ...
  social   仅社交媒体（Twitter + 微信公众号）
    ...
    parser.add_argument(
        "--mode",
        choices=["all", "news", "market", "social"],
        default="all",
        help="运行模式 (默认: all)"
    )
    ...
    if args.mode == "news":
        ...
        _run_market(mode="market")
    elif args.mode == "social":
        _run_market(mode="social")
    elif args.mode == "all":
        # 先跑市场数据，再跑热榜新闻
        _run_market(mode="all")
        _run_news()
